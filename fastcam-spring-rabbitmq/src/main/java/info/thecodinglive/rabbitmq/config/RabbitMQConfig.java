...
public class RabbitMQConfig {
    ...
    @Bean
    public DefaultClassMapper classMapper() { // 어떠한 클래스로 메시지를 주고 받을 것인지 지정할 수 있다.
    ...
    @Bean
    public MessageConverter rabbitMessageConverter() { // 메시지를 JSON으로 컨버팅해서 사용할 수 있게 설정한다.
        Jackson2JsonMessageConverter jsonConverter = new Jackson2JsonMessageConverter(FastcamJacksonConverter.getInstance());
        jsonConverter.setClassMapper(this.classMapper());
        return jsonConverter;
    }
    
    @Bean
    public ConnectionFactory rabbitConnectionFactory() { // 연결할 정보를 설정한다.
    ...
    public RabbitAdmin rabbitAdmin(ConnectionFactory rabbitConnectionFactory) {
        ...
        // exchange 등록
        this.rabbitExchange(rabbitAdmin);
        // queue 자동 등록
        for (FastcamRabbitQueue fastcamRabbitQueue : FastcamRabbitQueue.values()) {
            ...
            rabbitAdmin.declareBinding(BindingBuilder.bind(new Queue(fastcamRabbitQueue.getQueueName(), true))
                .to(this.rabbitExchange(rabbitAdmin)).with(fastcamRabbitQueue.getQueueName()));
        }
